...
function mod_studentquiz_helper_attempt_stat_joins($cmid, $groupid, $excluderoles = []): \core\dml\sql_join {
    ...
    $groupjoinquestioncreatebyuser = mod_studentquiz_helper_get_group_joins($groupid);
    $join .= "
    ...
    $groupjoinapprovedquestion = mod_studentquiz_helper_get_group_joins($groupid);
    $join .= "
        -- Approved questions.
        LEFT JOIN (
                    SELECT count(*) AS countq, q.createdby AS creator,
    ...
    $groupjoinratingsql = mod_studentquiz_helper_get_group_joins($groupid);
    $join .= "
    ...
    $groupjoinattemptsql = mod_studentquiz_helper_get_group_joins($groupid);
    $join .= "
        LEFT JOIN (
    ...
    $groupjoinstatstsql = mod_studentquiz_helper_get_group_joins($groupid);
    $join .= "
                  GROUP BY sp.userid
    ...
    $groupjoingsql = groups_get_members_join($groupid, 'u.id', \context_module::instance($cmid));
...
/**
 * Get group joins for creating sql. If $groupid = 0, return empty sql_join to reduce the complication of the sql.
 *
 * @param int $groupid Group id
 * @param string $groupidcolumn Group id column for the where clause
 * @return \core\dml\sql_join The joins clause will be empty in this case, we just return the wheres and params.
 */
function mod_studentquiz_helper_get_group_joins($groupid = 0, $groupidcolumn = 'sqq.groupid') {
    static $i = 0;
    $i++;
    $alias = 'gid' . $i;
...
function mod_studentquiz_question_stats($cmid, $groupid) {
    ...
    if ($groupid) {
        $groupjoinsql = mod_studentquiz_helper_get_group_joins($groupid);
