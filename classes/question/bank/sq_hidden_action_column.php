    ...
    protected function get_url_icon_and_label(\stdClass $question): array {
        ...
        if (has_capability('mod/studentquiz:previewothers', $this->qbank->get_most_specific_context())) {
            if ($question->sq_hidden) {
                $url = new \moodle_url('/mod/studentquiz/hide.php',
                        ['questionid' => $question->id, 'sesskey' => sesskey(), 'courseid' => $courseid, 'cmid' => $cmid]);
                return [$url, 't/show', get_string('show')];
            } else {
                $url = new \moodle_url('/mod/studentquiz/unhide.php',
                        ['questionid' => $question->id, 'sesskey' => sesskey(), 'courseid' => $courseid, 'cmid' => $cmid]);
